...
public class PluginUtil {
    ...
    public static ComponentName getComponent(Intent intent) {
        ...
        if (isIntentFromPlugin(intent)) {
            ...
            try {
                targetPackage = intent.getStringExtra(Constants.KEY_TARGET_PACKAGE);
            } catch (Exception e) {
                e.printStackTrace();
            }
            String targetActivity = "";
            try {
                targetActivity = intent.getStringExtra(Constants.KEY_TARGET_ACTIVITY);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return new ComponentName(targetPackage, targetActivity);
        }
    ...
    public static boolean isIntentFromPlugin(Intent intent) {
        ...
        boolean extra = false;
        try {
            extra = intent.getBooleanExtra(Constants.KEY_IS_PLUGIN, false);
    ...
    public static void copyNativeLib(File apk, Context context, PackageInfo packageInfo, File nativeLibDir) throws Exception {
        Log.d(TAG, "copyNativeLib() called with: apk = [" + apk + "], context = [" + context + "], packageInfo = [" + packageInfo + "], nativeLibDir = [" + nativeLibDir + "]");
